...
const index = () => {
  ...
  useEffect(() => {
    ...
    return () => {
      // Show the status bar again when unmounting
      StatusBar.setHidden(false);
    };
  }, []);

  const { bookingData, loading, error } = useBooking(String(bookingId));
  ...
  const {
    ...
    bookingData?.type === "Apartment" ? bookingData?.propertyId : ""
  );
  ...
  const {
    ...
    bookingData?.type === "Transient" ? bookingData?.propertyId : ""
  );
  ...
  const handleSubmitReview = async () => {
    try {
      ...
      if (bookingData?.type === "Apartment" && apartment) {
      ...
      } else if (bookingData?.type === "Transient" && transient) {
        await updateTransient(
          bookingData?.propertyId,
          {
            ...transient,
            reviews: [
        ...
        setErrorMsg("Unable to determine property type.");
      }
    } catch (error) {
          ...
          <View style={[styles.container]}>
            ...
            <View style={styles.center}>
              <View style={styles.mainContent}>
                ...
                />
                <Text style={styles.text}> Your Booking is Completed </Text>
                <Text style={styles.subText}> Please rate your stay. </Text>
              </View>
...
const styles = StyleSheet.create({
  ...
  container: {
    ...
    // Removed minHeight: 500 that was causing issues
  },
  backgroundVector: {
    height: "90%",
    width: "100%",
